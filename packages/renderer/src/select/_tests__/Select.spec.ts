...
import MockCustomSelectGroup from '../_fixtures__/MockCustomSelectGroup.svelte';
import MockCustomSelectOption from '../_fixtures__/MockCustomSelectOption.svelte';
import mountSelect, { fireEvent, screen, waitFor } from '../_fixtures__/mountSelect';

describe('Select', () => {
  it('should show a custom placeholder', async () => {
    await mountSelect.andInitialize({
      setSelectItems: {
        items: [],
  ...
  it('should show a message if the list is empty', async () => {
    await mountSelect.andInitialize({
      setSelectItems: {
        items: [],
      },
  ...
  it('should show a custom no results message', async () => {
    await mountSelect.andInitialize({
      setSelectItems: {
        items: [],
  ...
  it('should render custom options', async () => {
    await mountSelect.andInitialize({
      service: {
        CustomSelectOption: {
          component: MockCustomSelectOption,
        },
      },
      serviceId: 'mock',
      setSelectItems: {
  ...
  it('should render custom option groups', async () => {
    await mountSelect.andInitialize({
      service: {
        CustomSelectGroup: {
          component: MockCustomSelectGroup,
        },
      },
      serviceId: 'mock',
      setSelectItems: {
  ...
  it('should select the option when clicked', async () => {
    const { appApi, userEvent } = await mountSelect.andInitialize({
      setSelectItems: {
        items: [
          { id: 'one', label: 'One' },
          { id: 'two', label: 'Two' },
    ...
    expect(appApi.selectChange).toHaveBeenCalledWith('two');
  });

  it('should fuzzy filter options', async () => {
    const { userEvent } = await mountSelect.andInitialize({
      setSelectItems: {
        items: [
  ...
  it('should pass the window orientation when updating the bounds', async () => {
    const { appApi, emitAppApiEvent, userEvent } = await mountSelect.andInitialize({
      setSelectItems: {
        items: [
          { id: 'bulbasaur', label: 'Bulbasaur' },
          { id: 'ivysaur', label: 'Ivysaur' },
  ...
  it('should reset the state when the select is hidden', async () => {
    const { emitAppApiEvent, userEvent } = await mountSelect.andInitialize({
      setSelectItems: {
        items: [
          { id: 'charmander', label: 'Charmander' },
          { id: 'charizard', label: 'Charizard' },
  ...
  it('should scroll the window to the top or bottom of the active option', async () => {
    ...
    window.scrollBy = vi.fn() as unknown as Window['scrollBy'];
